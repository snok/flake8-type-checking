"""
This file tests the TYO100 error:
...
examples = [
    ...
    # ast.Import
    (f'import {mod}\nx: {mod}', {'1:0 ' + TYO100.format(module=f'{mod}')}),
    (f'\nimport {mod}\nx: {mod} = 2', {'2:0 ' + TYO100.format(module=f'{mod}')}),
    # ast.ImportFrom
    (f'from {mod} import Plugin\nx: Plugin', {'1:0 ' + TYO100.format(module=f'{mod}.Plugin')}),
    (f'\n\nfrom {mod} import constants\nx: Plugin = 2', {'3:0 ' + TYO100.format(module=f'{mod}.constants')}),
    # Aliased imports
    (f'import {mod} as x\ny: x', {'1:0 ' + TYO100.format(module=f'x')}),
    (f'from {mod} import constants as x\ny: x = 2', {'1:0 ' + TYO100.format(module='x')}),
    # ------------------------------------------------------------------------------------
    # Imports used for ast.arg annotation
    # ast.Import
    (f'import {mod}\ndef example(x: {mod}):\n\tpass', {'1:0 ' + TYO100.format(module=f'{mod}')}),
    (f'\nimport {mod}\ndef example(x: {mod} = 2):\n\tpass', {'2:0 ' + TYO100.format(module=f'{mod}')}),
    # ast.ImportFrom
    (f'from {mod} import Plugin\ndef example(x: Plugin):\n\tpass', {'1:0 ' + TYO100.format(module=f'{mod}.Plugin')}),
    (
        f'\n\nfrom {mod} import constants\ndef example(x: Plugin = 2):\n\tpass',
        {'3:0 ' + TYO100.format(module=f'{mod}.constants')},
    ),
    # Aliased imports
    (f'import {mod} as x\ndef example(y: x):\n\tpass', {'1:0 ' + TYO100.format(module=f'x')}),
    (f'from {mod} import constants as x\ndef example(y: x = 2):\n\tpass', {'1:0 ' + TYO100.format(module='x')}),
    # ------------------------------------------------------------------------------------
